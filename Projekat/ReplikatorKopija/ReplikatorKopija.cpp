...
void handleProcess(PROCESS* proces);
...
int main() 
{
	...
	iResult = getaddrinfo(NULL, PORT_RK, &hints, &resultingAddress);
	if (iResult != 0)
	{
	...
	printf("Waiting connection with ReplikatorOriginal...\n");
	...
	{
		printf("Connection with ReplikatorOriginal established.\n");
		...
		else if (FD_ISSET(*acceptedSocket, &readfds))
		{
			...
			if (iResult > 0)
			{
				if (recvbuf[0] == '+')
				{
					...
					puts("__________________________________________________________________________________");
					printf("Messages received from ReplikatorOriginal: %s. ID: {" GUID_FORMAT "}\n", &recvbuf[37], GUID_ARG(guid));
				}
				else if (recvbuf[0] == '*')
				{
					GUID guid = stringToGUID(&recvbuf[1]);
					...
					puts("__________________________________________________________________________________");
					printf("Request for restore data has been sent. ID: {" GUID_FORMAT "}\n", GUID_ARG(guid));
				}
				else if (recvbuf[0] == 'x')
				{
					puts("__________________________________________________________________________________");
					printf("ReplikatorOriginal closed connection with process: %s.\n", &recvbuf[1]);
				...
				else
				{
					...
					puts("__________________________________________________________________________________");
					printf("Process registered on ReplikatorOriginal, ID: {" GUID_FORMAT "}\n", GUID_ARG(id));

					//POKRETANJE NOVOG PROCESA
					STARTUPINFO si;
					PROCESS_INFORMATION pi;

					wchar_t Command[] = L"C:\\Users\\Korisnik\\Desktop\\protokoli\\Projekat\\x64\\Debug\\ProcesKopija.exe 25002";
					...
					if (!CreateProcess(_T("C:\\Users\\Korisnik\\Desktop\\protokoli\\Projekat\\x64\\Debug\\ProcesKopija.exe"),   // No module name (use command line)
